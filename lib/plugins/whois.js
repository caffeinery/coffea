...
function whois(target, fn, stream_id) {
    var self = this;
    if (!stream_id) {
      stream_id = utils.extractNetwork(target);
      if (!stream_id) stream_id = '0'; // TODO: send to all streams?
    }
    if (fn) {
    ...
    this.write('WHOIS ' + utils.targetString(target), stream_id);
}
    ...
    return function (irc) {
        ...
        irc.on('data', function (msg, stream_id) {
            ...
            case 'RPL_WHOISUSER':
                ...
                target = params[1];
                user = irc.getUser(target, stream_id);
