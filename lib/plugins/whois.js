...
function whois(target, fn, stream_id) {
    var self = this;
    if (!stream_id) stream_id = '0';
    if (fn) {
        this.whoisCallbacks[stream_id + '_' + target] = function (err, data) {
            delete self.whoisCallbacks[stream_id + '_' + target];
            fn(err, data);
        };
    }
    this.write('WHOIS ' + target, stream_id);
}
    ...
    return function (irc) {
        ...
        irc.on('data', function (msg, stream_id) {
            ...
            case 'RPL_ENDOFWHOIS':
                ...
                cb = irc.whoisCallbacks[stream_id + '_' + target];
                if (cb) {
                    cb(null, map[target]);
                } else {
                    // irc.emit('whois', null, map[target]);
                    utils.emit(irc, stream_id, 'whois', {user: map[target]})
                }
                break;
            case 'ERR_NEEDMOREPARAMS':
                ...
                if (cb) {
                    ...
                    utils.emit(irc, stream_id, 'whois', {}, new Error("Not enough parameters"));
                }
                break;
            case 'ERR_NOSUCHSERVER':
                ...
                if (cb) {
                    ...
                    utils.emit(irc, stream_id, 'whois', {}, new Error("No such server"));
                }
                break;
            case 'ERR_NOSUCHNICK':
                params = msg.params.split(' ');
                ...
                if (cb) {
                    ...
                    utils.emit(irc, stream_id, 'whois', {}, new Error("No such nick/channel"));
                }
                break;
            }
        });
