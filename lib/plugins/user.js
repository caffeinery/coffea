    ...
    return function (irc) {
        ...
        irc.getUser = function (nick, network) {
            ...
            var user = _.find(userCache, function (usr) {
                if (network) return (usr.getNick() === nick) && (usr.getNetwork() === network);
                else return usr.getNick() === nick;
            });
            if (user === undefined) {
        ...
        irc.on('join', function (event, network) {
            //add user to channellist
            event.channel.names[event.user.getNick()] = [];
        });

        irc.on('part', function (event, network) {
            ...
            irc.getChannellist().forEach(function (channelName) {
                _.each(irc.getChannel(channelName, network).getNames(), function (mode, nick, list) {
        ...
        irc.on('kick', function (event, network) {
            //remove user from channellist
        ...
        irc.on('quit', function (event, network) {
